...
def score(potential, siteA, siteB, atomsA, atomsB, alnA, alnB, label=None):
    max_distance = 15.0
...
def runner(alignments, potential, template):
    ...
    for alnA in alignments[tplA]:
        ...
        for alnB in alignments[tplB]:
            ...
            label = tplA + "+" + tplB
            scores = score(potential, site1, site2, atomsA, atomsB, alnA, alnB, label)
            if scores is None:
                continue
...
def predict():
    ...
    if debug:
        matches_fname = get_root() + "/Alignments/matches_human_05_prof_debug.tab"
    ...
    with open("/Users/agoncear/projects/Interactome/Workflow/Interactomes/export.csv") as f:
        for line in islice(f, 1, None):
            p1, _, p2 = line.strip().split(",")[5][1:-1].split(" ")
            selected_proteins.add(p1)
            selected_proteins.add(p2)
