...
def load_complex_types(fname):
    ...
    with open(fname) as f:
        for line in islice(f, 1, None):
            tpl, pdb, chA, chB, protA, protB, c_type, _ = line.strip().split("\t", 7)
...
def load_alignments(fname, bs_cov_threshold=0.7, aln_identity_threshold=0.2, sel_proteins=None):
...
def score(potential, siteA, siteB, atomsA, atomsB, alnA, alnB):
    ...
    for k, rA in enumerate(siteA):
        ...
        for l, rB in enumerate(siteB):
            ...
            distances[(k, l)] = bin_num

    def calc(resnA, resnB):
        total_score = .0
        summations = 0
        for k, a in enumerate(resnA):
            if a not in aa_short:
                continue
            for l, b in enumerate(resnB):
                if b not in aa_short:
                    continue
                d = distances.get((k, l))
                if d is None:
                    continue
                total_score += potential[a+b][d]
                summations += 1
        return total_score, summations

    rA = [alnA.site[(s.resn, s.resi)][0] for s in siteA]
    rB = [alnB.site[(s.resn, s.resi)][0] for s in siteB]
    model_score, pairs_summed_up = calc(rA, rB)
    ...
    if pairs_summed_up == 0:
        ...
        return model_zscore, model_avg_score, model_score, .0, .0, .0
...
def predict(templates_fname, alignment_fname, matches_fname):
    ...
    # THRESHOLDS REMOVED:
    alignments = load_alignments(alignments_fname, bs_cov_threshold=0.0, aln_identity_threshold=0.0)  # , sel_proteins=selected_proteins)
        ...
        try:
            for results in per_template_results:
                for result in results:
                    ...
                    score_template_full = round(score_template_full, 4)
                    score_template = round(score_template, 4)
                    score = round(score, 4)
                    print queryA, queryB, score_template_full, score_template, score  # , tpl, query_type, template_type, s1, s2, s3, s4, s5, s6
                    output = (
                        queryA, queryB, tpl,
